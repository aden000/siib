...
<head>
    ...
    <title>Pelaporan Barang Masuk</title>
    <style>
        @font-face {
...
<body>
    <?php foreach ($data as $key => $d) : ?>
        <div class="content-page">
            ...
            <h3>Pelaporan Barang Masuk</h3>
            <table style="width: 50%; text-align: left; margin-bottom: 20px;">
                <tbody>
                    <tr>
                        <td>Oleh</td>
                        <td>: <?= $d['barang_masuk']['nama_user']; ?></td>
                    </tr>
                    <tr>
                        <td>Tanggal</td>
                        <td>: <?= Time::parse($d['barang_masuk']['tanggal_masuk'])->toLocalizedString("d MMMM yyyy - HH:mm") ?></td>
                    </tr>
                    <tr>
                        <td>Vendor</td>
                        <td>: <?= $d['barang_masuk']['nama_vendor']; ?></td>
                    </tr>
                    <tr>
                        <td>Semester</td>
                        <td>: <?= $d['barang_masuk']['semester_ke']; ?> - <?= $d['barang_masuk']['tahun']; ?></td>
                    </tr>
                </tbody>
            </table>
            ...
            <table style="width: 100%;" class="detailBarang">
                ...
                <tbody>
                    <?php if (empty($d['detail_barang_masuk'])) : ?>
                        <tr>
                            <td colspan="4" style="text-align: center;">Tiada data detail barang</td>
                        </tr>
                    <?php else : ?>
                        <?php foreach ($d['detail_barang_masuk'] as $key => $db) : ?>
                            <tr>
                                <td><?= $key + 1; ?></td>
